...
class ContSimpleNoisyImageGeneratorBlackSquareAndCircle(ImageGenerator):
    """
    ...
    """
    def __init__(self, num_images, image_size=(256, 256), square_size=50, minSize=8, maxSize=100):
        super().__init__(num_images, image_size, minSize=minSize, maxSize=maxSize)
    ...
    def generate_images(self, maxSize=100):
        for _ in range(self.num_images):
            ...
            # Draw the square and circle in black on the shapes image
            cv2.rectangle(shapes_img, (square_center[0] - self.square_size // 2, square_center[1] - self.square_size // 2),
                          (square_center[0] + self.square_size // 2, square_center[1] + self.square_size // 2), color=1, thickness=-1) # color is required but set in contours
            cv2.circle(shapes_img, circle_center, circle_radius, color=1, thickness=-1) # color is required but set in contours
            ...
            for cnt in contours:
                cv2.drawContours(background, [cnt], 0, 0, -1)
