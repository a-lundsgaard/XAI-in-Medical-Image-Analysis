...
class SliceAggregateTransform(MapTransform):
    def __init__(self, keys: KeysCollection, slices=3, allow_missing_keys: bool = False, views=['coronal', 'sagittal', 'axial']):
        self.slices = slices
    ...
    def slice_and_aggregate_3_channel(self, img_data):
        ...
        # return only the coronal slice

        #combined_image = np.stack((coronal_slice), axis=0)

        combined_image = np.expand_dims(coronal_slice, axis=0)
